...
class ConfigManager:
    ...
    DEFAULT_CONFIG = {
        "thresholds": {"mouth_open": 0.35, "eyebrows_raised": 0.28, "smile": 0.35},
        "actions": {
    ...
    def _sync_monitored_expressions(self):
        """ Ensures consistency across config sections based on thresholds keys. """
    ...
    def get_enabled_gestures(self):
        """Returns the 'enabled_gestures' dictionary."""
    ...
    def update_gesture_enabled(self, key, is_enabled):
        """Updates the enabled status for a single gesture and saves."""
